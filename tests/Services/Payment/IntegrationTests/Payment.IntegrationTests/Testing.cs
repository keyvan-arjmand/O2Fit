...
public partial class Testing
{
    ...
    public static IConsumerTestHarness<T> GetConsumerTestHarness<T>() where T : class, IConsumer
    {
        using var scope = _scopeFactory.CreateScope();

        var consumerTestHarness = scope.ServiceProvider.GetRequiredService<IConsumerTestHarness<T>>();
        return consumerTestHarness;
    }

    [OneTimeTearDown]
    public Task RunAfterAnyTestsAsync()
    {
        return Task.WhenAll(TestContainers.DisposeAsync());
    }
}