...
public interface IUserGenericRepository<T> where T : UserAggregateRoot
{
    ...
    Task<T?> GetSingleDocumentByFilterWithSortAsync(FilterDefinition<T> filter, Expression<Func<T, object>> sort,
      CancellationToken cancellationToken = default);
    Task<List<T>> GetListOfDocumentsByFilterAsync(FilterDefinition<T> filter,
        CancellationToken cancellationToken = default);

    Task<IAsyncCursor<T>> GetByFilterAsync(FilterDefinition<T> filter, FindOptions<T> options,
         CancellationToken cancellationToken = default);
    Task<PaginationResult<T>> GetAllPaginationAsync(int pageIndex, int pageSize, CancellationToken cancellationToken = default);
    ...
    Task<UpdateResult> SoftDeleteByIdAsync(ObjectId id, T aggregateRoot, UpdateOptions? options = null,
        CancellationToken cancellationToken = default);
