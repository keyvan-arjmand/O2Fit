    ...
    public static IServiceCollection AddWebApiServices(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddSwaggerGen(options =>
        {
            options.SwaggerDoc("v1", new OpenApiInfo
            {
                Version = "v1.0",
                Title = "Chat Api V1",
                Description = "Chat Api V1",
            });
            options.SwaggerDoc("v2", new OpenApiInfo
            {
                Version = "v2.0",
                Title = "Chat Api V2",
                Description = "Chat Api V2",
            });
        ...
        services.AddMassTransit(x =>
        {
            var entryAssembly = AppDomain.CurrentDomain.GetAssemblies()
                .FirstOrDefault(f => f.FullName!.Contains("Chat.Application"));
        ...
        services.AddOpenIddict()
            .AddValidation(options =>
            {
                ...
                options.AddAudiences(configuration["IdentitySettings:Audiences:ChatResource"]!);
