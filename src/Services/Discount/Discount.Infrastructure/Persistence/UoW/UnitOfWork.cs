...
namespace Discount.Infrastructure.Persistence.UoW;
...
public class UnitOfWork : IUnitOfWork
{
    ...
    public IDiscountRepository DiscountRepository() {
        ...
        var type = nameof(DiscountO2Fit);
    
        if (!_repositories.ContainsKey(type))
        {
            var repositoryType = typeof(DiscountRepository);
            var repositoryInstance =
                Activator.CreateInstance(repositoryType,  _mediator , _configuration,
                    _currentUserService);
            _repositories.Add(type, repositoryInstance);
        }
        return (IDiscountRepository)_repositories[type];
    }

    public IDiscountNutritionistRepository DiscountNutritionistRepository()
    {
        if (_repositories is null)
            _repositories = new Hashtable();
    
        var type = nameof(DiscountNutritionist);
    
        if (!_repositories.ContainsKey(type))
        {
            var repositoryType = typeof(DiscountNutritionistRepository);
            var repositoryInstance =
                Activator.CreateInstance(repositoryType,  _mediator , _configuration,
                    _currentUserService);
            _repositories.Add(type, repositoryInstance);
        }
        return (IDiscountNutritionistRepository)_repositories[type];
    }
