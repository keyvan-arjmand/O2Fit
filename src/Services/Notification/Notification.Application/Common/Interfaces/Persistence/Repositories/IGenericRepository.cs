namespace Notification.Application.Common.Interfaces.Persistence.Repositories;
...
public interface IGenericRepository<T> where T : AggregateRoot
{
    ...
    Task<List<T>> GetListOfDocumentsByFilterAsync(FilterDefinition<T> filter,
        CancellationToken cancellationToken = default);
    Task<List<T>> GetListPaginationOfDocumentsByFilterAsync(int pageIndex, int pageSize,FilterDefinition<T> filter,
        CancellationToken cancellationToken = default);
