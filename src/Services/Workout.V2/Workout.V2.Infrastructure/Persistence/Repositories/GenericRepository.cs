namespace Workout.V2.Infrastructure.Persistence.Repositories;
...
public class GenericRepository<T> : IGenericRepository<T> where T : AggregateRoot
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var totalCount = await GetListOfDocumentsByFilterAsync(filter,cancellationToken);
        var result = PaginationResult<T>.CreatePaginationResult(pageIndex, pageSize, totalCount.Count, documents);
        return result;
    }
