namespace Nutritionist.Infrastructure.Persistence.Repositories;
...
public class GenericRepository<T> : IGenericRepository<T> where T : AggregateRoot
{
    ...
    public async Task<PaginationResult<T>> GetListPaginationOfDocumentsByFilterAsync(int pageIndex, int pageSize,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return PaginationResult<T>.CreatePaginationResult(pageIndex, pageSize, data.Count, data);
    }
    ...
    public async Task InsertOneManualAuditLogAsync(T? aggregateRoot, InsertOneOptions? options = null,
        CancellationToken cancellationToken = default)
    {
        if (aggregateRoot is null)
            throw new ArgumentException("aggregateRoot cannot be null or empty.", nameof(aggregateRoot));
