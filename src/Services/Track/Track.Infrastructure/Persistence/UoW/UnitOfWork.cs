...
using Track.Infrastructure.Persistence.Repositories;

namespace Track.Infrastructure.Persistence.UoW;
...
public class UnitOfWork : IUnitOfWork
{
    ...
    public ITrackSpecificationRepository TrackSpecificationRepository()
    {
        if (_repositories is null)
            _repositories = new Hashtable();

        var type = typeof(TrackSpecification).Name;

        if (!_repositories.ContainsKey(type))
        {
            var repositoryType = typeof(TrackSpecificationRepository);
            var repositoryInstance =
                Activator.CreateInstance(repositoryType, _mediator, _configuration,
                    _currentUserService);
            _repositories.Add(type, repositoryInstance);
        }
        return (TrackSpecificationRepository)_repositories[type];
    }
