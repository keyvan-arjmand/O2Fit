...
{
    public class CreateFoodCommandHandler : IRequestHandler<CreateFoodCommand, int>, IScopedDependency
    {
        private readonly IFileService _fileService;
        private readonly IFoodRepository _foodRepository;

        public CreateFoodCommandHandler(IFileService fileService, IFoodRepository foodRepository)
        {
        ...
        public async Task<int> Handle(CreateFoodCommand request, CancellationToken cancellationToken)
        {
            ...
            if (request.FoodType != FoodType.Supermarket)
            {
                ...
                if (foodFactors.AfterBaking - foodFactors.DryIngredient <= 0
                    && (FoodType)request.FoodType != FoodType.Supermarket &&
                    (BakingType)request.BakingType != BakingType.NoTypeOfBaking)
                    throw new AppException(ApiResultStatusCode.BadRequest, "Your food is burnt");

                request.WeightBeforeBaking = request.SumWeight;
            ...
            var food = new Food
            {
                ...
                WeightBeforBaking = request.WeightBeforeBaking,
                GI = request.GI,
