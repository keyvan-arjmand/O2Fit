...
{
    public class GetTranslationQueryHandler : IRequestHandler<GetTranslationQuery, List<Translation>>, ITransientDependency
    {
        ...
        public async Task<List<Translation>> Handle(GetTranslationQuery request, CancellationToken cancellationToken)
        {
            ...
            {
                ListCheck.Add($"Translation_Identity_{item}");
            }
            ...
            if (_GetTranslations.Count == 0)
            {
                ...
                for (int i = 0; i < _GetTranslations.Count; i++)
                {
                    ...
                    keyValuePairs[i] = new KeyValuePair<RedisKey, RedisValue>($"Translation_Identity_{_GetTranslations[i].Id}", _json);
                }
